...
1. Identity providers **MUST** implement OAuth 2.1 with appropriate security measures
   (see [RFC9700](https://datatracker.ietf.org/doc/html/rfc9700)) for both confidential and
   public clients.
...
Servers **MUST** return the fully-qualified domain name of the issuer in `WWW-Authenticate`
headers in a *HTTP 401 Unauthorized* response.
...
1. Securely store Access and Refresh Tokens received from the identity provider.  
...
1. Servers **MUST** validate all incoming tokens in one of the three ways:
  ...
  1. JWT validation, according to [RFC9068](https://www.rfc-editor.org/rfc/rfc9068.html).
